...
import type { HouseholdDashboardReader, HouseholdDashboardData } from '../domain/ports.js';
...
export class DrizzleHouseholdDashboardReader implements HouseholdDashboardReader {
  ...
  async read(params: {
    ...
    const privacyMap = new Map<CategoryId, CategoryPrivacyLevel>(
      privacyRows.map((row) => [row.categoryId as CategoryId, row.level as CategoryPrivacyLevel]),
    );

    const txCategoryMap = new Map<TransactionId, CategoryId>(
      mappingRows.map((row) => [row.transactionId as TransactionId, row.categoryId as CategoryId]),
    );
