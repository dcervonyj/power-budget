...
import type { FxRateRepository } from '../../domain/index.js';
...
function makeRepo(getRate: FxRateRepository['getRate']): FxRateRepository {
  return {
    saveBatch: vi.fn(),
...
describe('GetFxRateUseCase', () => {
  ...
  it('throws FxRateUnavailableError when no rate found within 7 days', async () => {
    ...
    await expect(useCase.execute({ base: 'EUR', quote: 'USD', asOf })).rejects.toThrow(
  ...
  it('FxRateUnavailableError message includes base, quote, and date', async () => {
    ...
    await expect(useCase.execute({ base: 'EUR', quote: 'USD', asOf })).rejects.toThrow(
      'FX_RATE_UNAVAILABLE: EUR/USD for 2026-01-20',
    );
  });
});
